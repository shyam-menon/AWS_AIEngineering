...
class CustomerSupportUI:
        ...
        try:
            ...
            # Method 10: Final fallback - if agent explicitly mentions escalation/handoff in response
                  ...
                  any(phrase in agent_response.lower() for phrase in [
                      "escalat", "management team", "human agent", "supervisor", 
                      "connecting you", "handoff", "transferring you", "specialist",
                      "within 24 hours", "within 2 hours"  # More specific time indicators
                  ])):
                handoff_detected = True
            ...
            if handoff_detected:
                ...
                # Clear any stale human feedback for non-handoff scenarios
                if session_id in human_responses:
                    del human_responses[session_id]
                    print(f"🔄 Cleared stale human feedback for session: {session_id}")
                human_feedback = None
            ...
            # Use the handoff detection result directly (no need for duplicate detection)
            escalation_detected = handoff_detected
    ...
    def _extract_customer_response_from_conversation(self, conversation_text):
        ...
        # Method 1: Look for the final response after the last </thinking> tag
        thinking_end = conversation_text.rfind('</thinking>')
        if thinking_end != -1:
            # Get text after the last thinking section
            after_thinking = conversation_text[thinking_end + len('</thinking>'):].strip()
            
            # Clean up the response by removing debug info and system messages
            lines = after_thinking.split('\n')
            response_lines = []
            
            for line in lines:
                line = line.strip()
                # Skip debug messages, tool outputs, and system messages
                if (line and 
                    not line.startswith('🔍 DEBUG') and 
                    not line.startswith('📝 Agent response') and
                    not line.startswith('✅ Query processed') and
                    not line.startswith('Tool #') and
                    not line.startswith('👤') and
                    not line.startswith('⏳') and
                    not line.startswith('Agent requested user input:')):
                    response_lines.append(line)
            
            if response_lines:
                # Join the response lines and clean up
                response = '\n'.join(response_lines)
        ...
        # Method 2: Look for response generated by generate_customer_response tool
        ...
        # Method 3: Look for common response patterns
        response_patterns = [
            "Certainly!",
            "I sincerely apologize",

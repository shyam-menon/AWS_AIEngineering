...
@tool
def search_company_docs(query: str) -> list:
    """Searches company documentation using AWS Bedrock Knowledge Base."""
    bedrock_agent_runtime = boto3.client('bedrock-agent-runtime')
    ...
    try:
        ...
        for result in response['retrievalResults']:
            results.append({
                'content': result['content']['text'][:500] + "...",
                'score': result['score'],
...
comprehensive_agent = Agent(
    ...
    description="A helpful assistant that can check weather, search documents, and more",
    tools=[get_weather, search_company_docs],
    memory=ConversationBufferMemory(max_token_limit=2000),
    system_prompt="""You are a helpful company assistant with access to various tools.
...
### Advanced Strands Agent with Custom Memory

Let's create a more sophisticated agent with custom memory management:

```python
from strands import Agent
from strands.memory import BaseMemory
from strands.tools import tool
import json
import sqlite3
from datetime import datetime, timedelta
...
class PersistentMemory(BaseMemory):
    """Custom memory implementation that persists to SQLite database."""
    
    def __init__(self, db_path: str = "agent_memory.db", max_entries: int = 1000):
    ...
    def add_message(self, role: str, content: str, session_id: str = "default", metadata: Dict = None):
...
@tool
def analyze_code_repository(repo_name: str, analysis_type: str = "overview") -> dict:
    """Analyze a code repository for various metrics."""
    analyses = {
        "overview": {
            "repo_name": repo_name,
        ...
        "security": {
            ...
            "security_score": 8.5,
            "recommendations": [
                "Update dependency: requests to version 2.31.0",
    ...
    return analyses.get(analysis_type, analyses["overview"])

# Create advanced agent with persistent memory
advanced_agent = Agent(
    name="AdvancedEngineeringAssistant",
    description="An advanced AI assistant for engineering teams with persistent memory",
    tools=[analyze_code_repository, search_company_docs],
    memory=PersistentMemory(max_entries=5000),
    system_prompt="""You are an advanced AI assistant specifically designed for engineering teams.
    ...
    You should personalize responses based on past interactions."""
)
```

### Multi-Agent System with Strands

Now let's create a sophisticated multi-agent system where different agents specialize in different domains:
...
@tool
def analyze_financial_data(data_type: str, period: str = "current_quarter") -> dict:
    """Analyze financial data and metrics."""
    financial_data = {
        "revenue": {
            "current_quarter": {"value": 2500000, "growth": 15.2, "trend": "increasing"},
            "previous_quarter": {"value": 2170000, "growth": 8.1, "trend": "stable"}
        },
        "expenses": {
            "current_quarter": {"value": 1800000, "growth": 12.1, "trend": "controlled"},
            "previous_quarter": {"value": 1605000, "growth": 10.2, "trend": "increasing"}
        }
    }
    
    return financial_data.get(data_type, {"error": f"Data type '{data_type}' not found"})

@tool
def generate_marketing_insights(campaign_type: str, metrics: List[str] = None) -> dict:
    ...
    return insights.get(campaign_type, {"error": f"Campaign type '{campaign_type}' not found"})

# Create specialized agents
financial_agent = Agent(
    name="FinancialAnalyst",
    description="Specialized in financial analysis and business metrics",
    tools=[analyze_financial_data],
    system_prompt="""You are a financial analyst AI with deep expertise in business metrics, 
    financial analysis, and strategic planning. You provide data-driven insights and recommendations 
    based on financial data."""
)

marketing_agent = Agent(
    name="MarketingStrategist",
    description="Specialized in marketing analysis and campaign optimization",
    tools=[generate_marketing_insights],
    system_prompt="""You are a marketing strategist AI with expertise in digital marketing, 
    campaign analysis, and customer engagement. You provide insights on marketing performance 
    and strategic recommendations for improving marketing ROI."""
)

# Coordinator agent that can work with all specialists
@tool
def consult_specialist(domain: str, query: str) -> dict:
    ...
    if domain in specialists:
        response = specialists[domain](query)
        return {
            "domain": domain,
        ...
        return {"error": f"No specialist available for domain: {domain}"}

coordinator_agent = Agent(
    name="ExecutiveCoordinator",
    description="Coordinates between different specialist agents to provide comprehensive business insights",
    tools=[consult_specialist, analyze_financial_data, generate_marketing_insights],
    system_prompt="""You are an executive coordinator AI that works with specialist agents to provide 
    comprehensive business insights. You can consult with financial and marketing specialists 
    to answer complex business questions that require multi-domain expertise."""
)
```
